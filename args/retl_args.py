...
def retl_bulk_insert_args(retl_insert):
    retl_insert.add_argument('--sobject', '--sobj', type=str, required=True, help='The name of the table to create')
    retl_insert.add_argument('--query', '--q',help='the file that holds the query')
    retl_insert.add_argument('--username', '--u', type=str, required=True, help='The username you wish to use to login to Salesforce')
    retl_insert.add_argument('--db_connect','--db', type=str, required=False, help='The Snowflake database connection') 
    retl_insert.add_argument('--instance_type','--instance', type=int, required=True, help='Salesforce Instance Type:  1 = Productio; 0 = Sandbox')
    return retl_insert  #.parse_args()
...
def retl_update_args(retl_update):
    retl_update.add_argument('--sobject', '--sobj', type=str, required=True, help='The name of the table to update')
    retl_update.add_argument('--query', '--q',help='the file that holds the query')
    retl_update.add_argument('--username', '--u', type=str, required=True, help='The username you wish to use to login to Salesforce')
    retl_update.add_argument('--db_connect','--db', type=str, required=False, help='The Snowflake database connection') 
    retl_update.add_argument('--instance_type','--instance', type=int, required=True, help='Salesforce Instance Type:  1 = Productio; 0 = Sandbox')    
    return retl_update  
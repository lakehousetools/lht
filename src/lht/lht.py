...
def main():
    ...
    bulk_status_parser.add_argument('--job_type','--type', help='Filter by the type of job. Options are Classic, V2Ingest, V2Query.  If blank, will return all')
    #job types include:
    ...
    if args.command == 'create':
        ...
        sobject_create.create(snowflake_connection(f"{args.db_connect}"),access_token, f"{args.sobject}", f"{args.table}")
    elif args.command == 'sync':
        if args.lastmodifieddate is None:
            sobject_sync.new_changed_records(snowflake_connection(f"{args.db_connect}"), f"{args.sobject}", f"{args.table}",f"{args.sync_field}")
        else:
        ...
        access_token = auth.get_access_token()
        session = snowflake_connection(f"{args.db_connect}")
        bapi20.get_bulk_results(session, access_token,  f"{args.job_id}",  f"{args.sobject}", f"{args.schema}", f"{args.table}")
        ...
        bapi20_info = bapi20.query_status(access_token, job_type, f"{args.job_id}")
        print(bapi20_info)
